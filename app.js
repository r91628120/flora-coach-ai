  ...
  function initGeneratePage() {
    ...
    function renderResults(payload) {
      ...
      payload.directions.forEach((item) => {
        ...
        card.innerHTML = `
          <div class="design-image" style="background:${item.image};">
    ...
    function handleGenerate() {
      ...
      const directions = buildMockDirections(data);
      const payload = {
        form: data,
        directions,
        prompt: buildGeneratePrompt(data),
...
請用繁體中文回答，語氣專業、具教練感，重點是幫助學習者成長。`;
  ...
  function initJudgePage() {
    ...
    function setFormData(data = {}) {
      ids.forEach(id => { if (byId(id)) byId(id).value = data[id] || (id === 'judgeMode' ? 'general' : ''); });
    }
      ...
      scores.forEach(item => {
        ...
        card.innerHTML = `
          <div class="score-head">
            ...
            <div class="score-num">${item.value.toFixed(1)} / 10</div>
          </div>
    ...
    function buildReportText(result, data, total) {
      const scoreText = result.scores.map(item => `${item.key}：${item.value.toFixed(1)} / 10`).join('\n');
...
${result.coach}`;
    ...
    function renderReport(payload) {
      ...
      const total = result.scores.reduce((sum, item) => sum + item.value, 0).toFixed(1);
    ...
    function handleJudge() {
      ...
      const result = buildMockJudgeResult(form);
      const payload = { form, result, image, createdAt: new Date().toISOString() };
    ...
    function goOptimize() {
      ...
      const optimizeSeed = {
        ...
        mode: payload.form.judgeMode === 'market' ? 'market' : (payload.form.judgeMode === 'contest' ? 'contest' : 'balanced')
      };
    ...
    if (savedImage?.dataUrl) handleImagePreview(savedImage.dataUrl, savedImage.name ? `已選擇：${savedImage.name}` : '已載入先前作品圖片');
...
請生成一個比上一輪更成熟、更精準、更貼近目標的花藝設計版本。`;
  ...
  function initOptimizePage() {
    ...
    function handleOptimize() {
      ...
      const plan = buildOptimizationPlan(form);
      const payload = { form, plan, prompt: buildOptimizePrompt(form, plan), createdAt: new Date().toISOString() };
    ...
    if (judgePayload && !savedForm.theme) {
      ...
      savedForm.mode = judgePayload.form.judgeMode === 'market' ? 'market' : (judgePayload.form.judgeMode === 'contest' ? 'contest' : 'balanced');
    }
    const generateForm = load(STORAGE_KEYS.generateForm, {});
